<?php 
	...
	global $free_membership_level, $paid_membership_level;
	...
	if($current_user->membership_level->id == $free_membership_level) {	
		$confirmation_message 	= "<p>Thank you for registering with " . get_bloginfo('name') . ". Your " . get_bloginfo('name')  . " membership status is currently <strong>Inactive</strong>. <br /><br />" .
		"<a href='" . pmpro_url("checkout") ."?level=6'>Activate Your Membership Account &raquo;</a>". "</p>";
	} else {
		$confirmation_message 	= "<p>Your Membership is now <strong>Active!</strong> Thank you for activating your membership with " . get_bloginfo('name') . ".</p>";		
	}
...
<?php 
	...
	else 
	{
		...
		if($current_user->membership_level->id == $free_membership_level) {
			...
			echo $active_confirmation;
		}
	?>	
	<ul>
		<li><strong>Account Username:</strong> <?php echo $current_user->display_name?></li>
		<li><strong>Account Email:</strong> <?php echo $current_user->user_email?></li>
		<li><strong>Membership Status:</strong> <?php echo $current_user->membership_level->name?></li>
	</ul>	
<?php 
	} 
?>
...
<?php if($current_user->membership_level->id == $free_membership_level) { ?>
	<p align="center"><a href="<?php echo pmpro_url("checkout")?>?level=6">Activate Your Membership Account &raquo;</a></p>               
<?php } else { ?>
